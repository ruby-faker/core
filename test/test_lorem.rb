...
class TestFakerLorem < Test::Unit::TestCase
  ...
  def test_characters_negatives
    assert_equal "", @tester.characters(count: -1)
    assert_equal "", @tester.characters(count: (-2..-1))
    assert_equal "", @tester.characters(count: [-1, -2])
  end

  def test_characters_with_args
    100.times { assert @tester.characters(count: 500).length == 500 }
  end
  ...
  def test_supplemental_words
    @words = @tester.words(count: 10_000, supplemental: true)
  ...
  def test_exact_sentence_word_count
    assert_equal 2, @tester.sentence(count: 2, supplemental: false, random_words_to_add: 0).split(" ").length
  end

  def test_exact_count_param
    assert(@tester.characters(count: 2).length == 2)
  ...
  def test_range_count_param
    cs = @tester.characters(count: 2..5)
  ...
  def test_exclusive_range_count_param
    cs = @tester.characters(count: 2...3)
    ws = @tester.words(count: 2...3)
    ss = @tester.sentences(count: 2...3)
    ps = @tester.paragraphs(count: 2...3)
  ...
  def test_array_count_param
    cs = @tester.characters(count: [1, 4])
  ...
  def test_paragraph_char_count
    paragraph = @tester.paragraph_by_chars(count: 256)
