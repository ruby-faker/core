...
class TestFakerCompany < Test::Unit::TestCase
  ...
  def test_brazilian_company_number
    ...
    digit_sum = sample[0..11].chars.each_with_index.inject(0) { |acc, (digit, i)|
      factor = 2 + (3 - i) % 8
    ...
    digit_sum = sample[0..12].chars.each_with_index.inject(0) { |acc, (digit, i)|
      factor = 2 + (4 - i) % 8
      acc + digit.to_i * factor
    }
    remainder = digit_sum % 11
  ...
  def abn_checksum(abn)
    ...
    abn.split("").map(&:to_i).each_with_index.map { |n, i|
      (i.zero? ? n - 1 : n) * abn_weights[i]
    }.inject(:+)
  end

  def luhn_checksum(luhn)
    luhn_split = luhn.each_char.map(&:to_i).reverse.each_with_index.map { |n, i|
      x = i.odd? ? n * 2 : n
      x > 9 ? x - 9 : x
    }
