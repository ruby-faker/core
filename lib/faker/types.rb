...
module Faker
  class Types < Base
    ...
    class << self
      ...
      def rb_hash(keys = 1, type = random_type)
        {}.tap do |hsh|
          shuffle(translate("faker.lorem.words")).uniq.first(keys).each do |s|
            hsh.merge!(s.to_sym => type)
          end
        end
      end

      def complex_rb_hash(keys = 1)
        rb_hash(keys, random_complex_type)
      end

      def rb_array(length = 1)
        [].tap do |ar|
          length.times do
            ar.push random_type
          end
